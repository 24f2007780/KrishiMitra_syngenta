...
def rank(context: FarmerContext):
    ...
    **POST /rank**
    ...
    try:
        ...
        request = RankRequest(
            grower_id=context.profile.farmer_id,
            crop=crop_enum,
            pest=pest_enum,
...
@app.get("/products/{farmer_id}", response_model=RankResponse)
async def get_products(farmer_id: str):
    """
    Call GET /products/{farmer_id}
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            context_res = await client.get(f"http://localhost:8006/context/{farmer_id}")
        ...
        logger.exception(f"GET /products failed for {farmer_id}")
        raise HTTPException(status_code=500, detail=str(exc))
