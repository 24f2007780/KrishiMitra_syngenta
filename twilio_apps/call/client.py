...
def place_interactive_call(
    ...
    *,
    ...
    """
  Store ``context`` in memory, then dial ``to`` with TwiML URL
  ``{base}/voice/start?session_id=...``.

  Webhook server must already be running and reachable at ``public_voice_base_url``.
  """
    base = (public_voice_base_url or os.environ.get("PUBLIC_VOICE_BASE_URL") or "").rstrip("/")
    if not base and not dry_run:
        raise RuntimeError("Set PUBLIC_VOICE_BASE_URL or pass public_voice_base_url")
    ...
    start_url = f"{base}/voice/start?session_id={session_id}"
    ...
    call = Client(sid, token).calls.create(to=to, from_=from_num, url=start_url)
    return {
        "dry_run": False,
        "to": to,
        "from": from_num,
